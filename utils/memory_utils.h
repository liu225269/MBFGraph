...
static void write_to_file(int fd, unsigned char *output, unsigned long bytes_to_write) 
{
  ...
  while(bytes_to_write) {
    ...
    if(bytes_written == -1UL) {
      if(errno != EAGAIN) {
	    BOOST_LOG_TRIVIAL(fatal) << "Stream writeout unsuccessful:" << strerror(errno);
	    exit(-1);
      }
    }
    else {
...
static int read_from_file(int fd, unsigned char *input, unsigned long bytes_to_read) 
{
  ...
  while(bytes_to_read) {
    ...
    if(bytes_read == -1UL) {
      return -1;
      if(errno != EAGAIN) {
        BOOST_LOG_TRIVIAL(fatal) << "Stream readin unsuccessful:" << strerror(errno);
        exit(-1);
      }
    }
...
static int read_from_file_atomic(int fd, unsigned char *input, unsigned long bytes_to_read) 
{
  __sync_fetch_and_add(&stat_bytes_read, bytes_to_read);
  while(bytes_to_read) {
    unsigned long bytes_read;
