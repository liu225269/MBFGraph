...
template <typename F>
class conductance {
  static bool is_red(vertex_t v) { return ((v & 1) != 0); }
  static bool different(vertex_t a, vertex_t b) { return (((a ^ b) & 1) != 0); }

 public:
  static unsigned long split_size_bytes() { return 1; }

  static unsigned long split_key(unsigned char *buffer, unsigned long jump) {
    return 0;
  }

  static unsigned long vertex_state_bytes() { return 0; }
  ...
  static void postprocessing() {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "ALGORITHM::CONDUCTANCE::VALUE " << conductance_value;
  }
