...
namespace algorithm {
    namespace sg_simple {
        ...
        public:
            ...
            bfs_async_per_processor_data() : local_vertices_discovered(0), local_edges_explored(0), local_seen_update(false) {}
            ...
            bool reduce(per_processor_data **per_cpu_array, unsigned long processors) {
                ...
                for (unsigned long i = 0; i < processors; i++) {
                    bfs_async_per_processor_data * data = static_cast<bfs_async_per_processor_data *> (per_cpu_array[i]);
                    ...
                    global_seen_update = global_seen_update || data->local_seen_update;
        ...
        public:
            ...
            static void apply_one_update(unsigned char *vertex_state, unsigned char *update_stream, per_processor_data *per_cpu_data, bool loopback,
                ...
                if (vertices[vindex].bfs_parent == (vertex_t) - 1) {
                    static_cast<bfs_async_per_processor_data *> (per_cpu_data)->local_vertices_discovered++;
            ...
            static bool generate_update(unsigned char *vertex_state, unsigned char *edge_format, unsigned char *update_stream, per_processor_data *per_cpu_data,
                unsigned long bsp_phase) {
                vertex_t src, dst;
                F::read_edge(edge_format, src, dst);
                unsigned long vindex = x_lib::configuration::map_offset(src);
                struct bfs_vertex *vertices = (struct bfs_vertex *) vertex_state;
                if (vertices[vindex].bfs_parent != (vertex_t) - 1 && vertices[vindex].bsp_phase == bsp_phase) {
                    struct bfs_update * update = (struct bfs_update *) update_stream;
                    update->parent = src;
                    update->child = dst;
                    static_cast<bfs_async_per_processor_data *> (per_cpu_data)->local_edges_explored++;
                    static_cast<bfs_async_per_processor_data *> (per_cpu_data)->local_seen_update = true;
                    return true;
                } else {
                    return false;
                }
            ...
            static void postprocessing() {
                BOOST_LOG_TRIVIAL(info) << "ALGORITHM::BFS::VERTICES_DISCOVERED " << bfs_async_per_processor_data::vertices_discovered;
                BOOST_LOG_TRIVIAL(info) << "ALGORITHM::BFS::EDGES_EXPLORED " << bfs_async_per_processor_data::edges_explored;
            }
