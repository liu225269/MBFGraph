...
class bfs_per_processor_data : public per_processor_data {
 public:
  ...
  bfs_per_processor_data()
      : local_vertices_discovered(0), local_edges_explored(0) {}

  bool reduce(per_processor_data **per_cpu_array, unsigned long processors) {
    unsigned long cur_edges_explored = 0;
    for (unsigned long i = 0; i < processors; i++) {
      ...
      cur_edges_explored += data->local_edges_explored;
      data->local_edges_explored = 0;
    }
    BOOST_LOG_TRIVIAL(info)
        << "ALGORITHM::BFS::CUR_EDGES_EXPLORED " << cur_edges_explored;
    return false;
  }
...
class bfs {
 ...
 public:
  ...
  struct __attribute__((__packed__)) bfs_vertex {
    vertex_t bfs_parent;
    vertex_t bsp_phase:31;
    vertex_t changed:1;
  };

  static unsigned long split_size_bytes() { return sizeof(struct bfs_update); }
  ...
  static bool check_bsp(unsigned char *vertex_state, unsigned long vertex_index,
    ...
    struct bfs_vertex *vstate = (struct bfs_vertex *)vertex_state;
    if (vstate->bfs_parent != (vertex_t)-1 && vstate->bsp_phase == bsp_phase) {
      return true;
    }
    return false;
  }
  ...
  static bool need_init(unsigned long bsp_phase) { return (bsp_phase == 0); }

  static per_processor_data *create_per_processor_data(
      unsigned long processor_id) {
    return new bfs_per_processor_data();
  }
