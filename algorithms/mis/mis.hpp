...
class mis_per_processor_data : public per_processor_data {
 public:
  ...
  mis_per_processor_data() : voted_out(0) {}
...
class mis {
 ...
 public:
  static unsigned long vertex_state_bytes() { return sizeof(struct vertex); }

  static unsigned long split_size_bytes() { return sizeof(struct update); }
  ...
  static bool check_bsp(unsigned char* vertex_state, unsigned long vertex_index,
    ...
    // struct vertex* vertices = (struct vertex*) vertex_state;
    // struct vertex* v =
    // &vertices[x_lib::configuration::map_offset(vertex_index)];
    struct vertex* v = (struct vertex*)vertex_state;
    if (v->in_mis) {
      return true;
    }
    return false;
  ...
  static bool check_reset_changed(unsigned char *vertex_state, unsigned long vertex_index,
    unsigned long bsp_phase){
        return true;
    }
  ...
  static bool apply_one_update(unsigned char* vertex_state,
    ...
    struct vertex* vertices = (struct vertex*)vertex_state;
    struct vertex* v =
        &vertices[x_lib::configuration::map_offset(updt->target)];
    if (v->in_mis) {
      static_cast<mis_per_processor_data*>(per_cpu_data)->voted_out++;
      v->in_mis = false;
    }
    return true;
  }
  ...
  static void postprocessing() {
    BOOST_LOG_TRIVIAL(info)
        << "ALGORITHM::MIS::NOT_IN_MIS " << mis_per_processor_data::not_in_mis;
  }
};
