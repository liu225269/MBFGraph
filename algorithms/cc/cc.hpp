...
class cc_per_processor_data : public per_processor_data {
 public:
  ...
  double cc_neg_delta;
  cc_per_processor_data() : cc_neg_delta(0) {}
...
class cc {
 ...
 public:
  ...
  static bool check_bsp(unsigned char* vertex_state, unsigned long vertex_index,
    ...
    if (vstate->active_phase == bsp_phase) {
      return true;
    }
    return false;
  }
  ...
  static bool apply_one_update(unsigned char* vertex_state,
    ...
    if (u->component < v->component) {
      ...
      if (!v->merged) {
        static_cast<cc_per_processor_data*>(per_cpu_data)
            ->cc_neg_delta++;  // One component merged
        v->merged = true;
      }
	  v->changed = true;
      return true;
