...
class hyperanf_pp_data : public per_processor_data {
 ...
 public:
  ...
  hyperanf_pp_data()
      : local_neighbour_cnt(0), local_seed_reach(0.0f), local_seed(0) {
  ...
  bool reduce(per_processor_data **per_cpu_array, unsigned long processors) {
    ...
    BOOST_LOG_TRIVIAL(info)
        << "ALGORITHM::ANF " << (iteration - 1) << " " << std::fixed << sum;
    BOOST_LOG_TRIVIAL(info) << "ALGORITHM::ANF::SEED " << seed;
    BOOST_LOG_TRIVIAL(info)
        << "ALGORITHM::ANF::SEED_REACH " << std::fixed << seed_reach;
    iteration++;
...
class hyperanf {
 ...
 public:
  ...
  static unsigned long split_size_bytes() {
    return sizeof(struct hyperanf_update) + sizeof_hll_counter(&hll_param);
  }

  static unsigned long vertex_state_bytes() {
    return sizeof(hyperanf_vertex) + sizeof_hll_counter(&hll_param);
  }

  static hyperanf_vertex *index_vertices(unsigned char *vstream,
                                         unsigned long index) {
  ...
  static bool apply_one_update(unsigned char *vertex_state,
    ...
    hyperanf_vertex *vertex =
        index_vertices(vertex_state, x_lib::configuration::map_offset(u->dst));
  ...
  static bool generate_update(unsigned char *vertex_state,
    ...
    hyperanf_vertex *vertex =
        index_vertices(vertex_state, x_lib::configuration::map_offset(src));
    if (vertex->changed) {
      hyperanf_update *u = (hyperanf_update *)update_stream;
      memcpy(u->hll_ctr, vertex->hll_ctr, sizeof_hll_counter(&hll_param));
      u->dst = dst;
      return true;
    } else {
      return false;
  ...
  static bool init(unsigned char *vertex_state, unsigned long vertex_index,
                   unsigned long bsp_phase, per_processor_data *cpu_state) {
    ...
    if (bsp_phase == 0) {
      ...
      // rollup counters
      hyperanf_pp_data *pp_data = static_cast<hyperanf_pp_data *>(cpu_state);
      if (vstate->changed) {
  ...
  static bool check_bsp(unsigned char *vertex_state, unsigned long vertex_index,
                        unsigned long bsp_phase,
                        per_processor_data *cpu_state) {
    // BOOST_ASSERT_MSG(0, "Do not support this function!");
    // hyperanf_vertex *vertex = index_vertices(vertex_state,
    // x_lib::configuration::map_offset(vertex_index));
    hyperanf_vertex *vertex = (struct hyperanf_vertex *)vertex_state;
    if (vertex->changed) {
      return true;
    }
    return false;
  ...
  static bool check_reset_changed(unsigned char *vertex_state,
                                  unsigned long vertex_index,
                                  unsigned long bsp_phase) {
    return true;
  }

  static bool need_init(unsigned long bsp_phase) { return true; }
  ...
  static unsigned long min_super_phases() { return 1; }
};
