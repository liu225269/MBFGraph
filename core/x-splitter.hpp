...
namespace x_lib {
    ...
    public:
        ...
        filter(unsigned long items, unsigned long processors) : num_queues(processors) {
            work_items = (struct filter_entry *) map_anon_memory(items * sizeof (struct filter_entry), true, "Filter");
        ...
        unsigned long dq(unsigned long qid) {
            ...
            do {
                while ((avail = work_q_visible[to_steal_from].work_q) != NULL) {
                    if (__sync_bool_compare_and_swap(&work_q_visible[to_steal_from].work_q, avail, avail->next)) {
    ...
    bool x_split(unsigned long *index_in, unsigned char *buffer_in, unsigned long *index_aux, unsigned char *buffer_aux,
        unsigned long input_bytes, unsigned long fanout, unsigned long final_split_cnt, filter *workq) {
        ...
        while (num_in < final_split_cnt) {
            ...
            BOOST_ASSERT_MSG(num_out <= final_split_cnt, "Shift error!");
            ...
            for (unsigned long i = 0; i < num_in; i++) {
                ...
                BOOST_ASSERT_MSG(input_size % split_size_bytes == 0, "Misalinged substream!");
                indices[input][i] = 0;
                ...
                if (sorted) {
                    memcpy(output_stream + indices[1 - input][output_start], input_stream, input_size);
                } else {
                    unsigned long saved_start_index = indices[1 - input][output_start];
                    for (unsigned long j = 0; j < input_size; j += split_size_bytes) {
                        output_id = M::map(T::key(input_stream + j)) >> shift;
                        //printf("%lu %lu %x%x%x%x\n", output_id, T::key(input_stream + j), *(input_stream + j), *(input_stream + j + 1), *(input_stream + j + 2), *(input_stream + j + 3));
                        output_id = output_id & (num_out - 1);
                        memcpy(output_stream + indices[1 - input][output_id], input_stream + j, split_size_bytes);
                        indices[1 - input][output_id] += split_size_bytes;
                    }
                    /* Re-adjust offsets */
