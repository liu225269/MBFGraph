...
namespace x_lib {
    ...
    struct state_iter_work : public work_base {
        ...
        void operator()(unsigned long processor_id, configuration *config, x_barrier *sync, algorithm::per_processor_data *cpu_state, unsigned char *outbuf) {
            unsigned long partitions_per_cpu = config->cached_partitions / config->processors;
            unsigned long remainder = config->cached_partitions - partitions_per_cpu * config->processors;
            ...
            for (i = start, j = 0; j < count; i++, j++) {
                ...
                for (k = 0; k < vertices; k++) {
                    unsigned char * vertex = &state_buffer->buffer[state_buffer->index[0][i] + k * config->vertex_size];
                    A::state_iter_callback(superp, i, k, vertex, cpu_state, state_buffer->buffer);
                }
            }
        }
...
template<typename A, typename IN, typename OUT>
    struct meta_work : public work_base {
        unsigned long superp;
        memory_buffer *state;
        ...
        void operator()(unsigned long processor_id, configuration *config, x_barrier *sync, algorithm::per_processor_data *cpu_state, unsigned char *outbuf) {
            ...
            int BYTES_PER_BFS = BITS_PER_BF * 256 / 8;
            ...
            assert(stream_in->bufsize % BYTES_PER_BFS == 0);
            if(stream_in->bufsize / BYTES_PER_BFS <= 2 * config->processors){
                if(processor_id == 0){
                    callback_state.BF_offset = BF_offset;
                    callback_state.bufin = stream_in->buffer;
                    callback_state.bytes_in = stream_in->bufsize;
                    A::partition_callback(&callback_state);
                }
            } else {
                unsigned long chunk_size = (stream_in->bufsize / BYTES_PER_BFS / 
                    config->processors) * BYTES_PER_BFS;
                assert(chunk_size > 0);
                callback_state.bufin = stream_in->buffer + chunk_size * processor_id;
                callback_state.BF_offset = BF_offset + chunk_size * processor_id;
                if (processor_id != config->processors - 1) {
                    callback_state.bytes_in = chunk_size;
                } else {
                    callback_state.bytes_in = stream_in->bufsize - chunk_size * processor_id;
                }
                //printf("%ld: %ld/%ld\n", processor_id, chunk_size, stream_in->bufsize);
                A::partition_callback(&callback_state);
                //printf("%ld: %ld/%ld done\n", processor_id, chunk_size, stream_in->bufsize);
                assert(callback_state.bytes_in == 0);
            }
            sync->wait();
        }
    ...
    struct work : public work_base {
        ...
        bool flush_buffer(unsigned long processor_id, configuration *config, x_barrier *sync, algorithm::per_processor_data *cpu_state) {
            memory_buffer *loopback_buffer = stream_out;
            bool empty = false;
            sync->wait();
            if (stream_out->bufsize > 0) {
                make_index<OUT, map_super_partition_wrap>(stream_out, processor_id, config->super_partitions, sync);
                if (processor_id == 0) {
                    if (loopback) {
                ...
                if (loopback) {
                    ...
                    unsigned char *loopback_src = loopback_buffer->get_substream(processor_id, superp, &loopback_bytes);
                    ...
                    } while (!__sync_bool_compare_and_swap(&stream_out->bufsize, offset_start, offset_stop));
                    memcpy(stream_out->buffer + offset_start, loopback_src, loopback_bytes);
                    sync->wait();
                    make_index<OUT, map_cached_partition_wrap >(stream_out, processor_id, config->cached_partitions, sync);
                    stream_callback_state callback_state;
                    callback_state.superp = superp;
                    ...
                    while ((partition_id = stream_out->work_queues->dq(processor_id)) != ULONG_MAX) {
                        ...
                        for (unsigned long i = 0; i < config->processors; i++) {
                            callback_state.bufin = stream_out->get_substream(i, partition_id, &callback_state.bytes_in);
        ...
        void final_flush(unsigned long processor_id, configuration *config, x_barrier *sync, algorithm::per_processor_data *cpu_state) {
            bool empty;
            if (stream_out != NULL) {
                do {
        ...
        void append_buffer(unsigned char *buffer, unsigned long bytes,
            ...
            while (bytes) {
                ...
                unsigned long space = (MIN(stream_out->bufbytes - offset_start, bytes) / OUT::item_size()) * OUT::item_size();
                unsigned long offset_stop = offset_start + space;
                if (__sync_bool_compare_and_swap(&stream_out->bufsize, offset_start, offset_stop)) {
        ...
        void execute_callback(stream_callback_state *callback, unsigned long processor_id, configuration *config, x_barrier *sync,
            algorithm::per_processor_data *cpu_state, unsigned char *outbuf) {
            ...
            while (callback->bytes_in) {
                ...
                if (stream_out != NULL && callback->bytes_out > 0) {
                    append_buffer(outbuf, callback->bytes_out, processor_id, config, sync, cpu_state);
                    callback->bufout = outbuf;
                    callback->bytes_out = 0;
                }
            }
        ...
        void operator()(unsigned long processor_id, configuration *config, x_barrier *sync, algorithm::per_processor_data *cpu_state, unsigned char *outbuf) {
            ...
            stream_callback_state callback_state;
            callback_state.superp = superp;
            callback_state.bytes_out_max = (OUTBUF_SIZE / OUT::item_size()) * OUT::item_size();
            callback_state.cpu_state = cpu_state;
            callback_state.loopback = false;
            ...
            if (ingest != NULL) {
                ...
                make_index<IN, map_super_partition_wrap>(ingest, processor_id, config->super_partitions, sync);
                unsigned long ingest_bytes;
                unsigned char *ingest_src = ingest->get_substream(processor_id, superp, &ingest_bytes);
                unsigned long offset_start;
                unsigned long offset_stop;
                do {
                    offset_start = stream_in->bufsize;
                    offset_stop = offset_start + ingest_bytes;
                } while (!__sync_bool_compare_and_swap(&stream_in->bufsize, offset_start, offset_stop));
                memcpy(stream_in->buffer + offset_start, ingest_src, ingest_bytes);
                sync->wait();
                if (processor_id == 0) {
                    BOOST_ASSERT_MSG(stream_in->bufsize == ingest->bufsize, "Error in partitioning ingest !");
                    ingest = NULL;
                }
            } else {
                callback_state.ingest = false;
            ...
            if (stream_in != NULL) {
                make_index<IN, map_cached_partition_wrap >(stream_in, processor_id, config->cached_partitions, sync);
            }
            sync->wait();
            ...
            while ((partition_id = input_filter->dq(processor_id)) != ULONG_MAX) {
                ...
                if (stream_in != NULL) {
                    for (unsigned long i = 0; i < config->processors; i++) {
                        //printf("%lu %lu %lu %lu\n", processor_id, i, partition_id, stream_in->index[i][partition_id]);
                        callback_state.bufin = stream_in->get_substream(i, partition_id, &callback_state.bytes_in);
                        execute_callback(&callback_state, processor_id, config, sync, cpu_state, outbuf);
                    }
                } else {
                    callback_state.bufin = callback_state.state;
                    callback_state.bytes_in = config->vertex_size * config->state_count(superp, partition_id);
                    execute_callback(&callback_state, processor_id, config, sync, cpu_state, outbuf);
                }
                A::partition_post_callback(superp, partition_id, cpu_state);
            }
    ...
    public:
        ...
        x_thread(struct configuration* config_in, unsigned long processor_id_in, algorithm::per_processor_data *cpu_state_in)
          : config(config_in), cpu_state(cpu_state_in), processor_id(processor_id_in) {
    ...
    class streamIO {
        ...
        void make_config() {
            ...
            config->vertex_footprint = config->vertex_size + A::vertex_stream_buffer_bytes();
    ...
    public:
        ...
        streamIO() {
            ...
            make_config();
            buffers = new buffer_manager(config, config->buffer_size, config->max_buffers);
            state_buffer = new memory_buffer(config, config->max_state_bufsize());
            startup_ioqs(config);
        ...
        unsigned long open_stream(const char *stream_name, bool new_file, unsigned long io_queue_number, unsigned long stream_unit,
        ...
        void terminate() {
            ...
            BOOST_LOG_TRIVIAL(info) << "CORE::UTILS::BYTES_WRITTEN " << stat_bytes_written;
        ...
        void set_bitset_stream(unsigned long stream, std::vector<unsigned char, boost::alignment::aligned_allocator<unsigned char, 4096> > *bitset){
            streams[stream]->set_bitset(bitset);
        }
        
        void clear_bitset_stream(unsigned long stream){
            streams[stream]->clear_bitset();
        }
        
        void restart_stream(unsigned long stream, unsigned long super_partition) {
            streams[stream]->restart(super_partition);
        }
        ...
        bool do_stream(streamIO<B> *sio, unsigned long superp, unsigned long stream_in, unsigned long stream_out, filter *override_input_filter, bool loopback);
        template<typename B, typename IN, typename OUT> friend
        bool do_meta_stream(streamIO<B> *sio, unsigned long superp, unsigned long stream_in, unsigned long stream_out, filter *override_input_filter, bool loopback);
...
#define SETUP_STREAMOUT()     \
  do {        \
    ...
    template<typename A, typename IN, typename OUT>
    static bool do_stream(streamIO<A> *sio, unsigned long superp, unsigned long stream_in, unsigned long stream_out, filter *override_input_filter,
        bool loopback = false) {
        ...
        if (stream_in != ULONG_MAX) {
            ...
            while (!sio->streams[stream_in]->stream_eof(superp)) {
                ...
                work_item.stream_in = sio->streams[stream_in]->read(superp);
            ...
            BOOST_ASSERT_MSG(override_input_filter != NULL, "Must have input stream or input filter !");
            work_item.input_filter = override_input_filter;
            work_item.stream_in = NULL;
            work_item.bufm = sio->buffers;
            BOOST_ASSERT_MSG(stream_out != ULONG_MAX, "Must have input stream or output stream !");
            work_item.stream_out = sio->buffers->get_buffer();
            work_item.disk_stream_out = sio->streams[stream_out];
            work_item.io_clock = &sio->io_wait_time;
            sio->workers[0]->work_to_do = &work_item;
        ...
        reduce_result = sio->cpu_state_array[0]->reduce(sio->cpu_state_array, sio->config->processors);
        return reduce_result;
    }
    
    template<typename A, typename IN, typename OUT>
    static bool do_meta_stream(streamIO<A> *sio, unsigned long superp, unsigned long stream_in, unsigned long stream_out, filter *override_input_filter,
        bool loopback = false) {
        struct meta_work<A, IN, OUT> work_item;
        bool reduce_result;
        work_item.superp = superp;
        work_item.state = sio->state_buffer;
        work_item.loopback = loopback;
        work_item.ingest = NULL;
        work_item.BF_offset = 0;
        BOOST_ASSERT_MSG(stream_in != ULONG_MAX, "Must have input stream !");
        BOOST_ASSERT_MSG(sio->ingest_buffers[stream_in] == NULL, "I do not implement anything of this !");
        while (!sio->streams[stream_in]->stream_eof(superp)) {
            sio->io_wait_time.start();
            work_item.stream_in = sio->streams[stream_in]->read(superp);
            unsigned long bufsize = work_item.stream_in->bufsize;
            sio->io_wait_time.stop();
            if (override_input_filter != NULL) {
                work_item.input_filter = override_input_filter;
            } else {
    ...
    static bool do_cpu(streamIO<A> *sio, unsigned long superp) {
        ...
        sio->workers[0]->work_to_do = &work_item;
        (*sio->workers[0])();
        bool reduce_result = sio->cpu_state_array[0]->reduce(sio->cpu_state_array, sio->config->processors);
        return reduce_result;
    }
