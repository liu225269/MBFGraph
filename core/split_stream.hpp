...
class ioq {
 ...
 public:
  ioq(const char *mnt_pt_in) : mnt_pt(mnt_pt_in) {
    queue = NULL;
    terminate = false;
  }
  ...
  const char *get_mnt_pt() { return mnt_pt; }
};
...
class memory_buffer {
 ...
 public:
  ...
  memory_buffer(configuration *details, unsigned long bufbytes_in)
      : refcount(0),
    ...
    buffer = (unsigned char *)map_anon_memory(bufbytes, true, "memory buffer");
#ifdef PYTHON_SUPPORT
    ...
    if (aux_buffer == NULL && !use_qsort) {
      aux_buffer =
          (unsigned char *)map_anon_memory(bufbytes, true, "memory buffer");
#ifdef PYTHON_SUPPORT
      auxpBuffer =
          PyArray_SimpleNewFromData(1, dim, NPY_UINT8, (void *)aux_buffer);
#endif
    }
  }
  ...
  void set_bufsize(unsigned long bytes_available,
    ...
    unsigned long stream_units = bufsize / stream_unit_bytes;
    bufsize = stream_units * stream_unit_bytes;
    // printf("bufbyte: %u stream_unit: %u stream_unit_bytes: %u\n", bufbytes,
    //    stream_units, stream_unit_bytes);
  }
  ...
  void reset_index(unsigned long processor_id) {
    ...
    memset(index[processor_id], 0, max_partitions * sizeof(unsigned long));
  ...
  unsigned long get_refcount() { return refcount; }
  ...
  unsigned char *get_substream(unsigned long processor, unsigned long partition,
                               unsigned long *size) {
    ...
    if (partition < (index_entries - 1)) {
      *size = (index[processor][partition + 1] - index[processor][partition]);
    } else {
  ...
  friend void make_index(memory_buffer *stream, unsigned long processor_id,
                         unsigned long partitions, x_barrier *thread_sync);
};

template <typename T, typename M>
static void make_index(memory_buffer *stream, unsigned long processor_id,
                       unsigned long partitions, x_barrier *thread_sync) {
  if (stream->indexed) {
    thread_sync->wait();
    return;
  ...
  if (!stream->use_qsort) {
    twizzle =
        x_split<T, M>(stream->index[processor_id], stream->buffer,
                      stream->aux_index[processor_id], stream->aux_buffer,
                      stream->per_cpu_sizes[processor_id],
                      stream->config->fanout, partitions, stream->work_queues);
  } else {
    if (processor_id == 0) {
      qsort_keys = partitions;
    }
    thread_sync->wait();
    x_split_qsort<T, M>(stream->index[processor_id], stream->buffer,
                        stream->per_cpu_sizes[processor_id],
                        stream->work_queues);
    twizzle = false;
  }
...
class buffer_manager {
 ...
 public:
  buffer_manager(configuration *config, unsigned long bufbytes,
...
class disk_stream {
  ...
  unsigned long sub_buf_size;
  unsigned long num_subreq;
  struct iocb **cbs;
 ...
 public:
  ...
  // reopen the file in read mode for hdfs_fuse

  void refreshRDfd(int superp) {
    if (vm.count("fuse")) {
      close(superp_fd[superp]);
      superp_fd[superp] =
          open(superp_fname[superp].c_str(), O_RDWR | O_LARGEFILE, S_IRWXU);
    }
  }

  // add these two interface such that Stream_IO can close and reopen the file
  // in write mode
  ...
  void refreshWRfd(int superp) {
    if (vm.count("fuse")) {
      close(superp_fd[superp]);
      superp_fd[superp] =
          open(superp_fname[superp].c_str(),
               O_RDWR | O_LARGEFILE | O_CREAT | O_TRUNC, S_IRWXU);
    }
  }

  disk_stream(const char *fname, bool new_file, bool is_compressed,
              ioq *io_queue_in, buffer_manager *bufm_in,
              unsigned long superp_cnt_in, unsigned long stream_unit_bytes_in,
      ...
      : superp_cnt(superp_cnt_in),
        ...
        events(NULL),
        random_bitset(NULL),
        random_pos((unsigned int)-1) {
    sub_buf_size = vm["page_size"].as<unsigned long>();
    BOOST_ASSERT_MSG(0 < sub_buf_size && ((sub_buf_size % 512) == 0),
                     "need page size and at least 512B");
    ...
    for (unsigned long i = 0; i < superp_cnt; i++) {
      disk_pages[i] =
          (unsigned char *)map_anon_memory(DISK_PAGE_SIZE, true, "Disk page");
      std::stringstream full_name;
      full_name << io_queue->get_mnt_pt() << "/";
      if (superp_cnt == 1) full_name << fname;
      else full_name << fname << "." << (superp_cnt + i);

      if (vm.count("no_dio") > 0) {
        superp_fd[i] =
            open(full_name.str().c_str(),
                 O_RDWR | O_LARGEFILE | (new_file ? (O_CREAT | O_TRUNC) : 0),
                 S_IRWXU);
      } else {
      ...
      if (superp_fd[i] == -1) {
        BOOST_LOG_TRIVIAL(fatal)
            << "Unable to open stream file:" << strerror(errno);
        exit(-1);
      }
      disk_pos[i] = 0;
      if (new_file) {
      ...
      if (compressed) {
        ...
        flip_buffers[i]->ready =
            (unsigned char *)map_anon_memory(max_io_unit, true, "zlib buffer");
        flip_buffers[i]->pending =
            (unsigned char *)map_anon_memory(max_io_unit, true, "zlib buffer");
    ...
    cbs = new struct iocb *[num_subreq];
    for (unsigned int i = 0; i < num_subreq; ++i) {
      cbs[i] = new struct iocb;
    }
    events = new struct io_event[num_subreq];
  }
  ...
  void set_bitset(
      std::vector<unsigned char,
                  boost::alignment::aligned_allocator<unsigned char, 4096> >
          *bitset) {
    BOOST_ASSERT_MSG(random_pos == (unsigned int)-1,
                     "Set bitset error (in use)!");
    random_bitset = bitset;
    random_pos = find_first(random_bitset);
    BOOST_ASSERT_MSG(random_pos != (unsigned int)-1,
                     "Cannot find any set bits!");
  }

  void clear_bitset() {
    BOOST_ASSERT_MSG(random_pos == (unsigned int)-1,
                     "clear bitset error (in use)!");
    random_bitset = NULL;
  }
  ...
  memory_buffer *read(unsigned long superp) {
    ...
    if (!eof(superp)) {
      ...
      write_op = false;
      io_queue->add_work(this);
    } else if (superp_cnt == 1 && current_pos == 0 &&
               random_bitset == NULL) {  // Add to cache
  ...
  void read_one_shot(memory_buffer *buffer, unsigned long superp) {
    one_shot = true;
    request = buffer;
    request_superp = superp;
    request->uptodate = false;
  ...
  void write_one_shot(memory_buffer *buffer, unsigned long superp) {
    one_shot = true;
    request = buffer;
    request_superp = superp;
    request->uptodate = false;
  ...
  void ingest(memory_buffer *ingest_buffer) {
    ...
    if (cache != NULL) {
      ...
      unsigned long space = MIN(cache->bufbytes - cache->bufsize, bytes);
      ...
      if (space > 0) {
        ...
        memcpy(cache->buffer + cache->bufsize,
               ingest_buffer->buffer + (ingest_buffer->bufsize - space), space);
    ...
    for (unsigned long i = 0; i < superp_cnt; i++) {
      ...
      set_filepos(superp_fd[i],
                  (disk_pos[i] / DISK_PAGE_SIZE) * DISK_PAGE_SIZE);
    }
    append(ingest_buffer);
  }
  ...
  void rewind(bool drop_cache = false) {
    ...
    if (cache == NULL) {
      ...
      BOOST_ASSERT_MSG(request == NULL, "Request pending while block cached !");
      if (request_superp == -1UL) {
        for (unsigned long i = 0; i < cache->config->processors; i++) {
          cache->reset_index(i);
        }
  ...
  void reset(unsigned long superp) {
    ...
    if (disk_bytes[superp] > 0) {
      ...
      // for libhdfs and libhdfs3 reopen the file in WRITEONLY mode implies
      // truncating the file
      if (vm.count("fuse")) {
        close(superp_fd[superp]);
        superp_fd[superp] =
  ...
  void restart(unsigned long superp) {
    request_superp = -1UL;
    eof_flag = false;
    if (cache != NULL) {
      cache->dirty = false;
