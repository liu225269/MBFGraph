...
namespace x_lib {
    ...
    struct configuration {
        ...
        unsigned long state_count(unsigned long superp, unsigned long partition) {
            unsigned long base = (partition << super_partition_shift) | superp;
            unsigned long count = (vertices - 1) & ~((1UL << (partition_shift + super_partition_shift)) - 1);
        ...
        unsigned long calculate_ram_budget() {
            ...
            buffer_size -= (buffer_size % DISK_PAGE_SIZE);
    ...
    public:
        ...
        static unsigned long map_inverse(unsigned long super_partition, unsigned long partition, unsigned long offset) {
            return (((offset << partition_shift) | partition) << super_partition_shift) | super_partition;
        }
