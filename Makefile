...
UTILS_SRC= utils.o
LIBS_SRC= core.o $(UTILS_SRC) bitshuffle_core.o iochain.o
...
CXXFLAGS?= -O3 -mavx -mavx2 -Wall -Wno-unused-function --std=c++11
#CXXFLAGS?= -g
...
#System libraries
SYSLIBS = -lboost_system -lboost_program_options -lboost_thread -lz -lrt -laio -lstdc++ -pthread#-lhdfs3
...
$(OBJECT_DIR)/core.o:core/core.cpp 
	...
	$(CXX) $(CXXFLAGS) -MM -MT '$(OBJECT_DIR)/core.o' $< > $(@:.o=.d)

$(OBJECT_DIR)/bitshuffle_core.o:core/bitshuffle_core.c 
	$(CXX) $(CXXFLAGS) $(EXTRA_INCLUDES) -c -o $@ $<
	$(CXX) $(CXXFLAGS) -MM -MT '$(OBJECT_DIR)/core.o' $< > $(@:.o=.d)

$(OBJECT_DIR)/iochain.o:core/iochain.c 
	$(CXX) $(CXXFLAGS) $(EXTRA_INCLUDES) -c -o $@ $<
	$(CXX) $(CXXFLAGS) -MM -MT '$(OBJECT_DIR)/core.o' $< > $(@:.o=.d)
...
$(OBJECT_DIR)/driver.o:benchmarks/driver.cpp 
	$(CXX) $(CXXFLAGS) $(EXTRA_INCLUDES) -c -o $@ $<
	$(CXX) $(CXXFLAGS) -MM -MT '$(OBJECT_DIR)/driver.o' $< > $(@:.o=.d)
...
bin/benchmark_driver:$(OBJECT_DIR)/driver.o $(LIBS)
	#$(CXX) $(CXXFLAGS) -o $@ $< $(LIBS) $(SYSLIBS) -Wl,--verbose 
